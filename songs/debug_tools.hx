...
function postCreate() {
    ...
    botplayTxt.visible = (FlxG.save.data.dev && curBotplay);
...
function update(elapsed:Float) {
	if (FlxG.save.data.dev) updateBotplay(elapsed);
	else return;
	...
	//i got tired of exiting the song ok -lean | Me too honestly xD -EstoyAburridow
	// ? -EstoyAburridow
	//if (FlxG.keys.justPressed.SEVEN) {
	//	Charter.__reload = true;
...
function onNoteHit(event) {
	if (!FlxG.save.data.dev || !curBotplay || event.character != boyfriend) return;
	...
	scripts.event("onPlayerHit", event);
}
